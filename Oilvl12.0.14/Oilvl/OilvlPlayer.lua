...
local quality_color = ITEM_QUALITY_COLORS
		...
		if cfg ~= nil and cfg.oilvlcharilvl and cfg.oilvlcharilvl ~= nil then
			...
			--for Key, Value in pairs(Items) do
						...
						if OItemAnalysis_CheckILVLGear4("player",Value) ~= 0 then							
							...
							if Value == 17 and OTCheckartifactwep(tonumber(itemID)) then
								...
								if totalilvl[Value] and totalilvl[16] and totalilvl[Value] > totalilvl[16] then
									...
									_G[Items[16].."Stock"]:SetTextColor(quality_color[quality].r,quality_color[quality].g,quality_color[quality].b)
							...
							quality_color = ITEM_QUALITY_COLORS
							if cfg.oilvlcolormatchitemrarity then
								Slot:SetTextColor(quality_color[quality].r,quality_color[quality].g,quality_color[quality].b)
							else
              ...
              ailvl = ailvl + (totalilvl[Value] or 0)
							...
							-- check no enchant
							if (Value == 1 or Value == 3 or Value == 5 or Value == 7 or Value == 8 or Value == 11 or Value == 12 or Value == 16) and enchant == "0" then
...
OiLvlPlayer.frame:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_ENTERING_WORLD" and OiLvlPlayer.set == 0 then
		...
		if not OiLvLCharFrame_Updatehooksw then
			PaperDollFrame:HookScript("OnShow", function() OiLvLPlayer_buttonsw(true) C_Timer.After(0.5,function() OiLvlPlayer_Update(true) end) end)			

...
function oilvlcheckrange()
	if not UnitAffectingCombat("player") and oilvlframesw then
		...
		if issecretvalue(oframe:GetName()) then return -1 end
		if type(oframe:GetName()) ~= "string" then return -1 end
...
function pppppppppppppp()
	local i
	local statisticCount = GetCategoryNumAchievements(15542)
	for i = 1, statisticCount do
		local IDNumber, OOName, _, _, _, _, _, _, _, _, _ = GetAchievementInfo(15542, i)
		print(OOName..":"..IDNumber)
	end
end

function qqqqqqqqqqqq()
	for n = 1300, 1400 do
		local name = EJ_GetInstanceInfo(n)
		if name then
			print(n, name)
		end
	end	
...
function OGetRaidProgression(RaidName, OSTAT, NumRaidBosses)
	...
	if rpunit == "" or not UnitExists("target") or orp["oframe"] == nil then
		OILVL:UnregisterEvent("INSPECT_ACHIEVEMENT_READY");
		--ClearAchievementComparisonUnit();
		rpsw=false;
		rpunit="";
...
function OGetRaidProgression2(RaidName, OSTAT, NumRaidBosses, Faction)
	...
	if rpunit == "" or rpunit == "target" or not UnitExists(rpunit) or not UnitExists(orp["unitid"]) then
		OILVL:UnregisterEvent("INSPECT_ACHIEVEMENT_READY");
		--ClearAchievementComparisonUnit();
		rpsw=false;
		rpunit="";
...
function OGetRaidProgression3(RaidName, OSTAT, NumRaidBosses, Faction)
	...
	if orp["oframe"] == nil then
		...
		if UnitExists(otooltip6rpdunit) and cfg.oilvlms then
			Omover2=2;
			ClearAchievementComparisonUnit();
			OILVL:RegisterEvent("INSPECT_ACHIEVEMENT_READY")
			rpsw=true;
	...
	if cfg.oilvlrpdetails then
		if LibQTip:IsAcquired("Oraidprog") or otooltip2 then
			...
			otooltip2:SetAutoHideDelay(0.25, self, function()
				...
				if not rpsw and UnitExists(otooltip6rpdunit) and cfg.oilvlms then
					Omover2=2;
					ClearAchievementComparisonUnit();
					OILVL:RegisterEvent("INSPECT_ACHIEVEMENT_READY")
					rpsw=true;
...
function oilvlSaveItemLevel(n)
	if OTCurrent == nil then return -1 end
	if OTCurrent2 == nil then return -1 end
	if OTCurrent3 == nil then return -1 end
	if OILVL_Unit ~= "" then
...
function events:INSPECT_READY(guid)
	...
	if cfg.oilvlms then
		--Omover=0;
		if not UnitAffectingCombat("player")  and cfg.oilvlms and UnitExists("target") and not UnitAffectingCombat("target") then
			local oname, _ = GameTooltip:GetUnit();
			if issecretvalue(oname) then return -1 end
			if oname ~= nil then oname = oname:gsub("%-.+", ""); else return -1; end
			if issecretvalue(oname) then return -1 end
			if issecretvalue(GetUnitName("target","")) then return -1 end
...
function events:INSPECT_ACHIEVEMENT_READY(...)
	if not UnitAffectingCombat("player")  and GetTime() - LastInspectATime > 2.5 then
		if cfg.oilvlms then
			if Omover2 == 1 then
				if UnitExists(rpunit) and rpsw then
			...
			elseif Omover2 == 2 then
				...
				if UnitExists("target") and rpsw then
...
function events:PLAYER_LOGIN(...)
	...
	if cfg.oilvlms then
		TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Unit, function(GameTooltip, data)
			if not UnitExists("target") or issecretvalue(GetUnitName("target","")) then return end
			local _, unit = GameTooltip:GetUnit()
            if issecretvalue(unit) then return end
			if unit ~= "target" then return end
			if issecretvalue(UnitIsUnit(unit, "target")) then return end
			if not unit or not UnitIsUnit(unit, "target") then return end
			OMouseover()
			local function upsertLine(prefix, value)
			if not value then return end
			for i = 2, GameTooltip:NumLines() do
				local fs = _G["GameTooltipTextLeft"..i]
				local txt = fs and fs:GetText()
				if txt and txt:find(prefix, 1, true) then
				fs:SetText(prefix..": |r|cFF00FF00"..value)
				return
				end
			end
			GameTooltip:AddLine(prefix..": |r|cFF00FF00"..value)
			end
			upsertLine(L["Item Level"], targetilvl ~= "" and targetilvl or nil)
		end)
	end
end
...
function OMouseover()
	...
	if not UnitAffectingCombat("player")  and cfg.oilvlms then
